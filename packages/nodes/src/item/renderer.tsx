...
const ModelRenderer = ({ node }: { node: ItemNode }) => {
  const modelUrl = resolveCdnUrl(node.asset.src)
  if (!modelUrl) return <BrokenItemFallback node={node} />

  return <LoadedModelRenderer modelUrl={modelUrl} node={node} />
}

const LoadedModelRenderer = ({ modelUrl, node }: { modelUrl: string; node: ItemNode }) => {
