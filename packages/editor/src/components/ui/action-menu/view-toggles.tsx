    ...
    <Popover onOpenChange={setIsOpen} open={isOpen}>
      <div className="flex items-center">
        ...
        <ActionButton
          ...
          label={`扫描模型：${showScans ? '显示' : '隐藏'}`}
          onClick={() => setShowScans(!showScans)}
          size="icon"
          variant="ghost"
        >
          <div className="relative">
            <img alt="扫描模型" className="h-[28px] w-[28px] object-contain" src="/icons/mesh.png" />
            <span className="absolute -right-1.5 -bottom-1 min-w-[14px] rounded-full bg-white/20 px-[3px] text-center font-medium text-[9px] text-white/70 leading-[14px]">
              {scans.length}
            </span>
          </div>
      ...
      >
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-medium text-foreground text-sm">扫描模型</p>
              {hasScans && (
                <p className="text-muted-foreground text-xs">
                  当前楼层共 {scans.length} 个扫描模型
                </p>
              )}
            </div>
            <UploadButton />
