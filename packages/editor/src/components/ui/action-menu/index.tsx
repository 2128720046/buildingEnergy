...
export interface ActionMenuProps {
  className?: string
  /**
   * floating: 保持原来的底部悬浮菜单。
   * embedded: 作为宿主前端右侧工具箱中的普通内容块嵌入。
   */
  layout?: 'floating' | 'embedded'
}

export function ActionMenu({ className, layout = 'floating' }: ActionMenuProps) {
  ...
  return (
    <TooltipProvider>
      <motion.div
        className={cn(
          layout === 'floating'
            ? 'fixed bottom-[200px] left-1/2 z-50 -translate-x-1/2'
            : 'relative z-0 w-full',
