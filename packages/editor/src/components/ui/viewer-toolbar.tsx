...
import { ChevronsLeft, ChevronsRight, Columns2, Moon, Sun } from 'lucide-react'
...
function LevelModeToggle() {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="bottom">
        楼层模式：{levelMode === 'manual' ? '手动' : levelModeLabels[levelMode]}
      </TooltipContent>
    </Tooltip>
  )
}
...
const wallModeConfig: Record<string, { icon: string; label: string }> = {
  up: { icon: '/icons/room.png', label: '完整墙体' },
  cutaway: { icon: '/icons/wallcut.png', label: '剖切墙体' },
  down: { icon: '/icons/walllow.png', label: '低墙' },
}
...
function WallModeToggle() {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="bottom">墙体显示：{config.label}</TooltipContent>
    </Tooltip>
  )
}
